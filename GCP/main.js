...
if( !config.token )
{
	console.log(chalk`{red.bold DOTOKEN1 is not defined!}`);
...
class DigitalOceanProvider
{
	...
	async listImages( )
	{
		...
		{
			for( let img of response.body.images)
			{
				// Print out
	...
	async dropletInfo (id)
	{
		...
		let response = await got(`https://api.digitalocean.com/v2/droplets/${id}`, { headers: headers, json:true })
							 .catch(err => console.error(`getDropletInfo ${err}`));
...
async function provision()
{
	...
	var name = "ssujal"+os.hostname();
	var region = "tor1"; // Fill one in from #1
